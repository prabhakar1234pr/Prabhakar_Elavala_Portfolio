...
// Browser speech types — not in default TS lib, declared here to satisfy tsc
...
export function VoiceWidget() {
  ...
  // Use unknown ref type — ISpeechRecognition is a local interface, not a built-in
  const recognitionRef = useRef<ISpeechRecognition | null>(null);
  const messagesEndRef  = useRef<HTMLDivElement | null>(null);
  const inputRef        = useRef<HTMLInputElement | null>(null);

  // ── Load TTS voices async (Chrome & Edge fire voiceschanged) ───────────────
  useEffect(() => {
  ...
  // ── Preferred TTS voice ────────────────────────────────────────────────────
  // Priority: Edge Neural > Chrome Neural > macOS > any en-US network > fallback
  const preferredVoice = useMemo(() => {
    ...
    return (
      ...
      voices.find((v) => /Microsoft.*Natural|Microsoft.*Online/i.test(v.name) && v.lang === "en-US") ||
  ...
  const speak = useCallback((text: string, onDone?: () => void) => {
    if (voiceMuted || !window.speechSynthesis) { onDone?.(); return; }
    window.speechSynthesis.cancel();
  ...
  const startListening = useCallback((onResult: (text: string) => void) => {
    ...
    rec.onend = () => setVStatus((s) => (s === "listening" ? "idle" : s));

    recognitionRef.current = rec;
    rec.start();
  }, []);

  // ── Fetch reply from /api/assistant ───────────────────────────────────────
  ...
  // ── Chat send with word-reveal streaming ──────────────────────────────────
  ...
  const handleVoiceInput = useCallback(async (text: string) => {
    ...
    try {
      ...
      setMessages((m) => [...m, { role: "assistant", content: reply }]);
      speak(reply, () => {
        if (autoListen) setTimeout(() => startListening(handleVoiceInput), 400);
      });
    } catch {
  ...
  const toggleMic = () => {
    if (vStatus === "listening") stopListening();
    else if (vStatus === "idle") startListening(handleVoiceInput);
  };
              ...
              <div className="px-4 pt-3 pb-5 border-t border-white/10 shrink-0">
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="flex items-center gap-5">
                    <button
                      onClick={() => { setVoiceMuted((v) => !v); if (!voiceMuted) window.speechSynthesis?.cancel(); }}
                      ...
                      {voiceMuted ? <VolumeX className="size-4 text-white/30" /> : <Volume2 className="size-4 text-white/60" />}
                    </button>
                  ...
                  <p className="text-[11px] text-white/25 text-center px-6">
                    {vStatus === "listening"  ? "Listening — tap mic to stop"  :
                     vStatus === "processing" ? "Getting response…"            :
                     vStatus === "speaking"   ? (autoListen ? "Will listen again when done" : "Playing response") :
                     autoListen ? "Auto-listen on — tap mic to start" : "Tap the mic and speak naturally"}
                  </p>
                </div>
              </div>
            </>
